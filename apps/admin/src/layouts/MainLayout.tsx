...
import { isAdmin, isStoreManager } from "../services/authService";
            ...
            <div className="flex items-center">
              ...
              <div className="ml-3 flex-1">
                ...
                <p className="text-xs text-gray-400">
                  {user?.groups.includes('super-admin') && 'システム管理者'}
                  {user?.groups.includes('store-manager') && !user?.groups.includes('super-admin') && '店舗管理者'}
                  {user?.groups.includes('staff') && user?.groups.length === 1 && 'スタッフ'}
                </p>
              </div>
            </div>
            <button
              onClick={logout}
              className="mt-3 w-full flex items-center justify-center px-3 py-2 text-sm text-gray-700 bg-gray-100 rounded-md hover:bg-gray-200 transition-colors"
            >
