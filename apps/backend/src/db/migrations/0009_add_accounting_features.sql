...
-- オプション・トッピングの価格フィールドをnumericに変更（整数の場合）
DO $$
BEGIN
    -- order_item_optionsのpriceカラムの型を確認し、integerの場合はnumericに変更
    IF EXISTS (
        ...
        ALTER TABLE "order_item_options" ALTER COLUMN "price" TYPE numeric(10, 2) USING "price"::numeric / 100;
    END IF;
    
    -- order_item_toppingsのpriceカラムの型を確認し、integerの場合はnumericに変更
    IF EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'order_item_toppings' 
        AND column_name = 'price' 
        AND data_type = 'integer'
    ) THEN
        ALTER TABLE "order_item_toppings" ALTER COLUMN "price" TYPE numeric(10, 2) USING "price"::numeric / 100;
    END IF;
END $$;
