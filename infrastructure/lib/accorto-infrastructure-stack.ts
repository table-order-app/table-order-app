        ...
        imageRepository: {
          ...
          imageConfiguration: {
            ...
            runtimeEnvironmentVariables: {
              NODE_ENV: 'production',
              PORT: '8080',
              DATABASE_URL: `postgres://{{resolve:secretsmanager:${dbCredentials.secretArn}:SecretString:username}}:{{resolve:secretsmanager:${dbCredentials.secretArn}:SecretString:password}}@${database.instanceEndpoint.hostname}:5432/accorto`,
              AWS_REGION: this.region,
              S3_BUCKET_NAME: imagesBucket.bucketName,
              CLOUDFRONT_URL: `https://${imagesDistribution.distributionDomainName}`,
              ALLOWED_ORIGINS: frontendApps.map(app => `https://${frontendDistributions[app].distributionDomainName}`).join(','),
              JWT_SECRET: '{{resolve:secretsmanager:accorto-jwt-secret:SecretString:secret}}', // 別途作成が必要
              BCRYPT_ROUNDS: '12',
              LOG_LEVEL: 'info',
            },
          },
